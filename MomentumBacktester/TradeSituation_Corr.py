...
class TradeSituation:
    ...
    __transaction_px: float

    def __init__(self, open_order_arg: Quote,
                 is_long_trade_arg: bool, take_profit_in_bps_arg: float,
                 transaction_px: float):
        # Check arguments sanity.
        if take_profit_in_bps_arg < 0:
            raise Exception("Please note that the take profit has to be positive (:2.2f)"
                            .format(take_profit_in_bps_arg))
        if transaction_px < 0:
            raise Exception("Your transaction price is negative?")
        # Init locals
        self.__is_closed = True
        self.__is_long_trade = is_long_trade_arg
        self.__max_dd_in_bps = transaction_px
        self.__pnl_bps = -transaction_px
        self.__executed_close_quote = None
        self.__executed_open_quote = None
        self.__take_profit_in_bps = take_profit_in_bps_arg
        self.__transaction_px = transaction_px
    ...
    def update_on_order(self, quote_arg: Quote) -> bool:
        ...
        new_pnl = self.calculate_pnl_and_dd(quote_arg)
        # Check if target pnl was reached
        if new_pnl >= self.__take_profit_in_bps:
    ...
    def calculate_pnl_and_dd(self, quote_arg: Quote) -> float:
        ...
        if self.__is_long_trade:
            self.__pnl_bps = -self.__transaction_px +\
                             quote_arg.price()\
                             - self.__executed_open_quote.price()
        else:
            self.__pnl_bps = -self.__transaction_px - \
                             quote_arg.price() \
                             + self.__executed_open_quote.price()
