...
class FifoDoublesList:
    ...
    def get_mean(self) -> float:
        ...
        list_sum = self.get_sum()
        return list_sum / self.__list_length_local
    ...
    def return_values(self) -> list:
        ...
        """
        returned_data_list = [None] * self.__list_length_local
        inserted_index = 0
        for index in range(self.__next_updated_index, self.__list_length_local):
            returned_data_list[inserted_index] = self.__data_list[index]
            inserted_index += 1
        for index in range(0, self.__next_updated_index):
            inserted_index += 1

        # Alternative method
        # returned_data_list = self.__data_list[self.__next_updated_index:self.__list_length_local] +\
        #    self.__data_list[0:self.__next_updated_index]
